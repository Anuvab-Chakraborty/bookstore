...
const AuthForm = ({ type = 'login' }) => {
  ...
  const handleTrialLogin = async (trialRole) => {
    try {
      const credentials = trialRole === 'user'
        ? { email: 'user@example.com', password:'user123' }
        : { email: 'recruiter@example.com', password:'recruiter123'};

      const res = await api.post(`/login/${trialRole}`, credentials);
      const token = res.data.access_token;
      ...
      toast.success('Logged in as trial ' + trialRole);
      navigate('/');
    } catch (err) {
      toast.error('Trial login failed');
    }
  };
        ...
        {!checkingToken && !submitted && (
          ...
          >
              ...
              {['user', 'seller'].map((r) => (
                <div key={r} className="relative group">
                  <button
                    type="button"
                    onClick={() => setRole(r)}
                    className={`cursor-pointer px-4 py-1 rounded-full border transition duration-300 hover:scale-105 ${
                      role === r ? 'bg-purple-600 text-white' : 'bg-white/10 text-white border-white/30'
                    }`}
                  >
                    {`Login as ${r.charAt(0).toUpperCase() + r.slice(1)}`}
                  </button>
                  <div className="absolute -top-8 left-full -ml-14 bg-black/80 text-xs px-2 py-1 rounded-md opacity-0 group-hover:opacity-100 transition">
                    {r === 'user' ? 'For renters or buyers' : 'For uploaders or shop owners'}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex justify-center gap-4 mb-6">
              <button
                type="button"
                onClick={() => handleTrialLogin('user')}
                className="text-sm bg-purple-500 hover:bg-purple-600 px-3 py-1 rounded-xl"
              >
                Try as User
              </button>
              <button
                type="button"
                onClick={() => handleTrialLogin('seller')}
                className="text-sm bg-purple-500 hover:bg-purple-600 px-3 py-1 rounded-xl"
              >
                Try as Seller
              </button>
